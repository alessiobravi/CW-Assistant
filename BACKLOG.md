...
| ID | Status | Item | Acceptance |
|---|---|---|---|
| PKG-001 | active | Produce signed Win64 installer | Hosted WiX/MSI generation, stable major-upgrade identity, numeric build revisions, branded executable/product icon, `CW Assistant` Start-menu program group, desktop shortcut, and stable download naming are implemented. The finish page offers to launch the app and defaults that option on only when an interactive upgrade found and closed a running instance; CI inspects the MSI tables for the detection, bounded close, conditional default, and launch target, normalizing its readable queries for Windows Installer's SQL dialect and retaining assertion failures in its queryable result. Clean Windows 11 install/upgrade/repair/uninstall tests, migration from the out-of-support WiX v3 toolchain, Authenticode signing, and signed update metadata remain. |
