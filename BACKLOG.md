...
| DSP-006 | todo | Replace the hard two-level keying envelope decision | The mark and space levels that every keying likelihood is measured against are estimated by assigning each frame to whichever level it is nearer -- a hard per-frame decision, the same kind the decoder above no longer makes, one stage earlier. At low SNR that assignment is close to a coin toss and every frame it gets wrong drags one level toward the other, so the likelihood reaching any decoder is already degraded. This is now the limiting stage: a duration-explicit segmenter is four times better than the threshold in isolation on matched evidence (0.061 against 0.236) yet only level end to end. Plain soft assignment is not the answer and is recorded so it is not retried -- weighting both levels by responsibility measured 0.3884 against 0.2946 and collapsed outright at 12 WPM and 12 dB, because near the middle both responsibilities sit near a half and an ambiguous frame pulls the levels together rather than leaving them alone. Whatever replaces it needs an explicit separation prior, and must hold the four silent captures at no false callsigns. |
