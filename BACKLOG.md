...
Last reviewed: 2026-09-09 (twenty-sixth entry) -- Official packages now enable
...
| SDR-001 | active | Add SoapySDR stream adapter | The RX-only adapter and official-package integration enumerate modules/devices, select the nearest supported sample rate, apply center frequency and AGC/manual gain, read CF32 channel 0, preserve absolute RF, and produce validated timestamped IQ blocks with distinct telemetry. Windows/macOS packages carry the redistributable runtime; Linux packages bundle it or declare the module dependency. Every staged package must load the RTL factory without hardware. Remaining: expose complete telemetry in Diagnostics, capture IQ under REC-001, measure wide-passband CPU limits, and complete physical-device acceptance. |
...
| SDR-003 | active | Validate SDRplay 3 | SDRplay uses the common SoapySDR RX boundary when the compatible external vendor API/service and SoapySDRPlay3 module are installed. CW Buddy preserves external module search paths but does not redistribute the proprietary vendor runtime. Remaining: document and pin accepted vendor API/module combinations, improve platform-specific discovery diagnostics, and pass live RSP acceptance on Windows, macOS, and Linux, including service/version mismatch, minimum sample rate, overload, reconnect, and selected-stream monitoring. |
