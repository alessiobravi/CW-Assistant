...
| DSP-002 | active | Detect and track candidate CW tones | The bounded full-passband bank provides sub-bin peaks, drift prediction, nearby-candidate suppression, numerical-floor and local-prominence guards, and short private-candidate expiry. Only candidates with repeated spectral observations plus at least three known Morse symbols, bounded unknown ratio, and adequate timing quality publish stable IDs/colors. Original samples feed phase-continuous 60/120/240 Hz paths, asymmetric noise references, centered-tone rejection, and automatic width selection. Add corpus-qualified controls, robust noise quantiles, and meet held-out false-channel, weak-signal, drift, and continuity targets. |
...
| PERF-001 | active | Build decoder accuracy/resource benchmark gate | A deterministic executable gates character edits/CER, acquired-WPM error, and no-CW false characters while reporting updates, simulated duration, wall time, real-time factor, hypothesis count, and allocated state. It covers 8/12/20/25/40/55 WPM, weak/jittered cases, and a 12→40 WPM transmission-gap change with 0/49 baseline edits; raw-bank tests cover simultaneous tones and adjacent rejection, while the threaded live fixture requires keyed Morse to publish a verified channel. Extend it with WER, call precision/recall, bandwidth-qualified SNR curves, co-channel separation, provisional/stable latency/revisions, peak memory, and per-platform overload behavior. |
