...
Last reviewed: 2026-09-09 (twenty-fifth entry) -- Direct SDR reception work
...
| SDR-001 | active | Add SoapySDR stream adapter | The optional RX-only adapter enumerates modules/devices, selects the nearest supported sample rate, applies center frequency and AGC/manual gain, reads CF32 channel 0, preserves absolute RF, and produces validated timestamped IQ blocks with distinct overflow, pipeline-overrun, timeout, discontinuity, invalid-block, and read-error telemetry. The live worker feeds the existing bounded receiver pipeline; the UI exposes explicit start/stop and actionable unavailable-backend diagnostics. Remaining: bundle or locate the SoapySDR runtime in official packages, expose complete telemetry in the diagnostics pane, capture IQ under REC-001, measure wide-passband CPU limits, and complete physical-device acceptance on each platform. |
| SDR-002 | active | Validate RTL-SDR | RTL-SDR is supported through the common SoapySDR RX boundary without another SDR application. Missing SoapyRTLSDR/runtime/device conditions are visible, and the hardware-independent adapter/stream contracts pass. Remaining: package or document the exact supported runtime/module combination and pass recorded plus live RTL-SDR acceptance on Windows, macOS, and Linux, including frequency accuracy, sustained overflow behavior, hot-unplug, and selected-stream monitoring. |
| SDR-003 | active | Validate SDRplay 3 | SDRplay is supported through the common SoapySDR RX boundary without another SDR application when the compatible external vendor API and SoapySDRPlay module are installed. Missing runtime/module/device conditions are visible and do not disable sound-card reception. Remaining: pin the supported vendor API/module versions and pass recorded plus live RSP acceptance on Windows, macOS, and Linux, including service/version mismatch, minimum supported sample-rate behavior, overload, reconnect, and selected-stream monitoring. |
