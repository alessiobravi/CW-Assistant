...
| UI-003 | active | Add clickable channel/callsign overlays | A stable-center/width colored area is the primary identification cue for each verified CW track, independent of adaptive carrier/filter changes, with a thinner keying-state line on top and an 18 px label enlarged to 32 px on hover. A direct topmost pointer target opens/reopens a larger scrollable/selectable decoded-text card; close leaves DSP active, drag reorders cards, and an open card follows same-frequency/color reacquisition under a replacement internal ID. A context-confirmed callsign becomes the prominent marker/card label; otherwise the marker identifies a CW stream by frequency. Pointer frequency movement is **not implemented yet**: add trace-area hit testing, normal-click local CW-slice centering, and a separate capability-checked, operator-confirmed radio RX retune action. |
