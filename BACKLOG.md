...
| CAT-003 | active | Implement split and transverter frequency domain | Checked integer-Hz RX/TX resolution, signed offsets, profile persistence, and CAT split contract are implemented; a compact decoder-panel faceplate shows grouped actual-RF RX/TX digits plus ON AIR, SIMPLEX/SPLIT, and CW/CW-R status. Exact RX entry and configurable stepping use a checked inverse RX offset before sending the provider dial frequency; TX frequency and mode remain read-only and unchanged. Retuning live RX re-centers tracked signals with sideband-aware mapping. OmniRig still lacks authoritative split/TX display, and provider-neutral TX/mode writes, setup preview, Doppler/satellite tracking, and hardware tests remain. |
