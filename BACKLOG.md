...
visual-only. Phase-aware narrowband evidence, confidence calibration,
same-frequency separation, and multiple-pass stages remain active backlog work.
...
| UI-003 | active | Add clickable channel/callsign overlays | Stable color-matched frequency markers now connect every tracked signal to its decoder row; add hit testing that maps labels/traces to absolute RF Hz and stable track IDs, left-click local-guide movement, and separately confirmed/capability-checked right-click RX CAT retuning using sideband and transverter mapping. |
...
| DSP-002 | active | Detect and track candidate CW tones | A bounded full-passband FFT local-peak bank provides stable frequency IDs/colors, nearby-candidate suppression, a 96 dB numerical-floor guard, and expiry; every live/WAV track now derives 500 Hz soft evidence from original samples using a phase-continuous mixer, three-stage 120 Hz narrowband filter, and adjacent noise references off the UI thread. Add sub-bin carrier/drift tracking, alternative automatic/configurable widths, robust noise quantiles, and meet documented false-channel, weak-signal, drift, and continuity targets on the corpus. |
...
| CW-001 | active | Implement explainable adaptive timing baseline | Every detected passband track converts independent raw narrowband SNR to smoothed key probability and evaluates nine bounded 8–60 WPM timing hypotheses. The quality-ranked leader remains provisional during acquisition, one adaptive path locks after sufficient evidence, and a bounded silent-gap reset handles transmission-to-transmission speed changes while preserving stable text. Letters/digits, common punctuation/prosigns, WPM, SNR, preliminary confidence, and provisional/stable text are published; add the full hidden semi-Markov/Viterbi path, per-character evidence, closer-spaced speed alternatives where measured, and held-out confidence calibration. |
...
| PERF-001 | active | Build decoder accuracy/resource benchmark gate | A deterministic executable gates character edits/CER, acquired-WPM error, and no-CW false characters while reporting updates, simulated duration, wall time, real-time factor, hypothesis count, and allocated state. It now covers 8/12/20/25/40/55 WPM, weak/jittered cases, and a 12→40 WPM transmission-gap change with 0/49 baseline edits; raw-bank tests cover simultaneous tones, adjacent rejection, and the threaded live path. Extend it with WER, call precision/recall, bandwidth-qualified SNR curves, co-channel separation, provisional/stable latency/revisions, peak memory, and per-platform overload behavior. |
...
| CALL-006 | active | Maintain frequency-anchored decoded observation lifecycle | Audio-frequency tracks now retain stable IDs/colors through keyed gaps, update their overlay/list row in place, and expire from both models after a bounded silent hold; add checked absolute RF Hz, a visible/configurable lost state, viewport-independent reacquisition, callsign-level identity, and profile-configurable retention. |
