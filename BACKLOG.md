...
| DSP-002 | active | Detect and track candidate CW tones | A bounded full-passband FFT local-peak bank now provides stable frequency IDs/colors, nearby-candidate suppression, independent timing state, and expiry; replace magnitude-bin association with phase-aware multirate narrowband evidence and meet documented false-channel, weak-signal, drift, and continuity targets on the corpus. |
...
| CW-001 | active | Implement explainable adaptive timing baseline | Every detected passband track now converts SNR to smoothed key probability, adapts timing with likelihood-weighted observations, and publishes letters/digits, common punctuation/prosigns, WPM, SNR, preliminary confidence, and separate provisional/stable text; add the full hidden semi-Markov/Viterbi hypothesis path, per-character evidence, broader speed acquisition, and held-out confidence calibration. |
...
| PERF-001 | active | Build decoder accuracy/resource benchmark gate | A deterministic executable now gates character edits/CER and no-CW false characters while reporting processed updates, simulated duration, wall time, real-time factor, and decoder object size for fixed 12/20/25 WPM weak/jitter cases; extend it with WER, call precision/recall, bandwidth-qualified SNR curves, co-channel separation, provisional/stable latency/revisions, peak memory, and per-platform overload behavior. |
