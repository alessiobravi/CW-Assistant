...
| CW-001 | active | Implement explainable adaptive timing baseline | Every detected passband track now converts independent raw narrowband SNR to smoothed key probability, adapts timing with likelihood-weighted observations, and publishes letters/digits, common punctuation/prosigns, WPM, SNR, preliminary confidence, and separate provisional/stable text; next add bounded multi-speed acquisition, then the full hidden semi-Markov/Viterbi hypothesis path, per-character evidence, and held-out confidence calibration. |
...
| PERF-001 | active | Build decoder accuracy/resource benchmark gate | A deterministic executable gates character edits/CER and no-CW false characters while reporting processed updates, simulated duration, wall time, real-time factor, and decoder object size for fixed 12/20/25 WPM weak/jitter cases; raw-bank tests now cover simultaneous tones, adjacent rejection, and the threaded live path. Extend it with WER, call precision/recall, bandwidth-qualified SNR curves, co-channel separation, provisional/stable latency/revisions, peak memory, and per-platform overload behavior. |
