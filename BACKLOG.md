...
than a fixed pixel width), with the keying-state line drawn thinner on top.
...
| CW-001 | active | Implement explainable adaptive timing baseline | Every detected passband track converts independent raw narrowband SNR to smoothed key probability and evaluates nine bounded 8–60 WPM timing hypotheses. The leader remains provisional during acquisition, one path locks after sufficient evidence, and a bounded silent-gap reset handles speed changes while preserving stable text. Letters/digits, punctuation/prosigns, WPM, SNR, cadence and timing quality, plus bounded per-character confidence/evidence are published; add the full hidden semi-Markov/Viterbi path, closer-spaced speed alternatives where measured, and held-out confidence calibration. **Fixed defect, found via real contest debug-capture data**: `timing_quality` and `mean_character_confidence` used to be mathematically forced identical (`CwTimingDecoder::finishCharacter()` fed the same per-character `confidence_` into both accumulators), so the two separately configured verification-gate thresholds were really gating on one blended signal, not independent evidence — confirmed directly against a real contest track whose combined confidence sat at 0.338 (under the 0.45 threshold) for its entire life despite visibly capturing a legible `TEST` mid-stream. `timing_quality` now accumulates a genuinely separate pure element-duration-ratio precision signal, excluding the amplitude/keying-probability component; `CwMultiSpeedDecoder::score()` was updated to keep scoring WPM-hypothesis selection on the blended `mean_character_confidence` (its original, already-tuned semantics) rather than the now-independent `timing_quality`, since an initial attempt to leave `score()` unchanged destabilized WPM lock on the existing deterministic test (traced and confirmed with an instrumented before/after comparison). Covered by a new core-test assertion that the two fields diverge. **Remaining, not yet fixed**: both are still lifetime-cumulative averages since track creation rather than a recent/windowed average, so a track's confidence can still stay dragged down by however garbled its earliest seconds were even once it reads cleanly now — a separate, larger change deferred pending its own validation pass. **Bigger architectural weakness identified, agreed to fix properly rather than patch**: `CwMultiSpeedDecoder` currently *locks* irrevocably onto one of the nine seed hypotheses once ~2.5s has elapsed and the leader has decoded as few as 2 symbols (`considerLock()`); once locked, the other eight hypotheses are never evaluated again, and the only recovery path is a full 2.5s silence gap that real contest/QSK traffic often never provides. A bad lock can be self-reinforcing (miscalibrated timing suppresses the very confidence signal the per-element adaptation gates on), reproducing exactly the "plausible but wrong, capped confidence" signature seen in real captures. Agreed direction: redesign so no hypothesis is ever irrevocably locked — keep hypotheses continuously re-evaluated for a track's whole life, require a decisive and sustained score margin before switching leaders (avoiding thrash), freeze already-published stable text on switch (generalizing the existing silence-triggered freeze), and periodically reseed long-losing hypotheses near the current leader's WPM to bound CPU cost. Must be validated against `PERF-001`'s existing CPU/state-size budgets. Not yet implemented. |
...
| PKG-004 | active | Add application update checking and guided install | A background check (disableable, ~4 s after startup) and a Settings → About **Check for updates** button compare the running version against the published manifest's `version` field; **Download update** fetches this platform's artifact and verifies its SHA-256 against `SHA256SUMS` before saving, discarding anything that fails; **Open installer**/**Show in folder** hand the verified download to the OS's own installer/package handler rather than installing silently. Remaining: silent self-install-and-relaunch (needs a separate updater helper process, since no process can safely replace its own running executable/bundle) is intentionally deferred until Windows Authenticode and macOS notarization signing land (`PKG-001`, `PKG-002`) — an unsigned auto-replaced macOS build would hit Gatekeeper friction immediately. |
