...
| UI-006 | done | Distinguish the CW guide from detected traces | The CW pitch guide is a bold line on the frequency axis only, not a vertical band, so it can no longer be mistaken for an identified signal; verified signal traces are the ones shown as a colored vertical area, remaining independently colored and clickable. |
