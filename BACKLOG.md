...
| UI-003 | active | Add clickable channel/callsign overlays | A stable-center/width colored area is the primary identification cue for each verified CW track, independent of adaptive carrier/filter changes, with a thinner keying-state line on top and a 26 px hover label. Confirmed structurally plausible callsign/frequency text belongs in the upper spectrum rather than over waterfall history. Marker click reliably opens/reopens a larger decoded-text card, close leaves DSP active, drag reorders cards, and an open card follows same-frequency/color reacquisition under a replacement internal ID. Pointer frequency movement is **not implemented yet**: add trace-area hit testing, normal-click local CW-slice centering, and a separate capability-checked, operator-confirmed radio RX retune action. |
