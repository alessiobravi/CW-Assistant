...
| CW-002 | active | Add compact causal learned likelihood path | An independent experimental causal GRU now predicts key-down and target-channel-CW probabilities only, with explicit recurrent state, deterministic training/evaluation, temporal-fragmentation metrics, anti-aliased WAV inference, and checked ONNX export. A dependency-free probability/event decoder preserves N-best acoustic uncertainty and append-only stable output. No model/runtime is shipped: compare TCN/GRU candidates, calibrate on a locked labeled receiver corpus, demonstrate character-level gain and hard-negative safety, quantize within the published state/resource budget, audit provenance, integrate optional per-architecture inference, and retain the deterministic fallback. |
