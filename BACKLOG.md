...
| VER-001 | done | Keep application and package versions consistent | One effective `major.minor.revision` value drives About, Qt identity, Windows executable/MSI metadata, macOS bundle metadata, Debian package, installed VERSION, CAT4OM identity, and the continuous manifest; hosted QML smoke verifies the rendered value. |
...
| UI-003 | active | Add clickable channel/callsign overlays | Stable vertical color-matched markers expose decoded callsign, AF/RF frequency, filter, and drift; click opens/reopens a session card, close leaves DSP active, and drag reorders open cards. Add trace-area hit testing, left-click local-guide movement, and separately confirmed/capability-checked right-click RX CAT retuning. |
...
| DSP-002 | active | Detect and track candidate CW tones | The bounded full-passband bank now provides sub-bin peaks, bounded carrier/drift prediction, stable IDs/colors, nearby-candidate suppression, numerical-floor guard, and expiry. Original samples feed phase-continuous parallel 60/120/240 Hz paths, asymmetric smoothed noise references, centered-tone rejection, and automatic width selection at 500 evidence updates/s. Add corpus-qualified configurable ranges, robust noise quantiles, and meet documented false-channel, weak-signal, drift, and continuity targets. |
...
| CALL-001 | active | Extract and rank callsign candidates | A conservative normalized letter+digit token is exposed on the track as soon as raw/provisional text permits; add ranked alternatives and measured precision/recall targets, including portable calls. |
