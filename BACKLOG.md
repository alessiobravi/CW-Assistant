...
| CALL-006 | active | Maintain frequency-anchored decoded observation lifecycle | Tracks retain stable IDs/colors through keyed gaps and silence, update overlays and operator-selected sessions in place, and expire after a profile-configurable hold (Settings → Display, default 30 s, maximum 300 s). A bounded frequency-color lease survives expiration for at least five minutes and follows known RX retunes, so a returning carrier reuses its color even with a new track ID. Linked live radio audio can show checked actual RF using provider state, transverter offset, CW pitch, and sideband direction; add a visible/configurable lost state, viewport-independent RF reacquisition, and callsign-level identity. |
