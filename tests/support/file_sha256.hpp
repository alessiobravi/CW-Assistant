...
inline std::string fileSha256(const std::string& path) {
  ...
  const auto transform = [&](const std::array<std::uint8_t, 64>& bytes) {
  ...
  const std::size_t tail = static_cast<std::size_t>(input.gcount());
  total_bytes += tail;
  block.fill(0U);
  input.clear();
  input.seekg(static_cast<std::streamoff>(total_bytes - tail));
  ...
  const std::uint64_t bits = total_bytes * 8U;
  for (std::size_t index = 0; index < 8U; ++index)
    block[63U - index] = static_cast<std::uint8_t>(bits >> (index * 8U));
  transform(block);
